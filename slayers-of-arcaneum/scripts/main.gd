...
func _spawn_mob():
	if mobs_spawned < max_mobs:
		var mob_scene
		if current_wave == 1:
			mob_scene = mob_scenes[0]
		elif current_wave == 2:
			mob_scene = mob_scenes[randi() % 2]
		else:
			mob_scene = mob_scenes[randi() % 3]
		...
		mobs_spawned += 1
		print("Mob spawned, total mobs: %d" % mobs_spawned)
		if mobs_spawned == max_mobs:
			spawn_timer.stop()

func _start_new_wave():
	print("Starting new wave: %d" % current_wave)
	mobs_spawned = 0
	spawn_timer.start()
...
func _on_mob_died(points):
	...
	mobs_spawned -= 1
	print("Mob died, remaining mobs: %d" % mobs_spawned)
	if mobs_spawned == 0:
